...
This repository shows how to send every currently known WhatsApp interactive / native flow button type using WhiskeySockets (Baileys fork) without modifying core source. The functionality is packaged and published as the npm package `phantom-buttons` which reproduces the binary node structure the official client emits so buttons render correctly for both private & group chats.
...
The enhanced functionality provided by the `phantom-buttons` package provides the missing functionality by:
...
```bash
npm install phantom-buttons
# or
yarn add phantom-buttons
```
...
```javascript
const { sendButtons } = require('phantom-buttons');
...
```javascript
const { sendInteractiveMessage } = require('phantom-buttons');
...
```javascript
const { sendTemplateButtons } = require('phantom-buttons');
...
```javascript
const { sendTemplateButtonsHydrated } = require('phantom-buttons');
...
### List Messages
```javascript
const { sendListMessage } = require('phantom-buttons');
...
### Cards (Carousel-like)
```javascript
const { sendCards } = require('phantom-buttons');
